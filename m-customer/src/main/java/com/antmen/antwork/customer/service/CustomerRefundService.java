...
public class CustomerRefundService {
    ...
    public void requestRefund(CustomerRefundRequestDto requestDto) {
        Payment payment = paymentRepository.findById(requestDto.getPayId()).orElseThrow(()->new NotFoundException("결제 정보를 찾을 수 없습니다."));
        if (refundRepository.existsByPayment_PayId(payment.getPayId())) {throw new IllegalArgumentException("이미 환불 요청이 존재합니다.");}
